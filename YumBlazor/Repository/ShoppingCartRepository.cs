...
namespace YumBlazor.Repository
{
    public class ShoppingCartRepository : IShoppingCartRepository
    {
        ...
        public async Task<int> GetTotalCartCountAsync(string? userId)
        {
            ...
            // FIXED: Using Sum directly is faster and safer
            return await _db.ShoppingCart.Where(u => u.UserId == userId).SumAsync(u => u.Count);
        }
        ...
        public async Task<bool> UpdateCartAsync(string userId, int productId, int updateBy)
        {
            ...
            if (cart == null)
            {
                cart = new ShoppingCart { UserId = userId, ProductId = productId, Count = updateBy };
                await _db.ShoppingCart.AddAsync(cart);
            }
            else
            {
                cart.Count += updateBy;
                if (cart.Count <= 0) _db.ShoppingCart.Remove(cart);
            }
            return await _db.SaveChangesAsync() > 0;
        }
    }
